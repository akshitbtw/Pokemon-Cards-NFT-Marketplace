...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function handleFormSubmit(event) {
        ...
        const file1 = [`{ "name" : "${name}","description" : {"Card Number" : "${cardnumber}","Rarity" : "${rarity}","Card Type" : "${cardtype}", "HP" : "${hp}", "Stage" : "${stage}","Card Text" : "${cardtext}","Attack 1" : "${attack1}","Attack 2" : "${attack2}","Weakness" : "${weakness}","Retreat Cost" : "${retreatcost}"},"image" : "https://ipfs.io/ipfs/${ipfsUrl1}/${filename}" }`]
